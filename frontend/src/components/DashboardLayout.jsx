...
export default function DashboardLayout({ children }) {
  ...
  const isActive = (path) => {
    ...
    const dashboardPaths = ["/student/dashboard", "/tpo/dashboard", "/admin/dashboard"];
    ...
    <div style={s.root}>
      ...
      <aside style={s.sidebar}>
        ...
        <div style={{ ...s.roleBadge, color: roleColor, borderColor: roleColor + "40", background: roleColor + "14" }}>
          {user?.role} {user?.isPrimary && (<span style={s.primaryTag}>Primary</span>)}
        </div>

        <nav style={s.nav}>
          {navItems.map((item) => (
            <div key={item.path} style={{...s.navItem, ...(isActive(item.path) ? {
                  color: roleColor,
        ...
        <div style={s.bottom}>
          ...
          <button style={s.changePassBtn} onClick={() => navigate("/change-password")}>Change Password</button>
...
const s = {
  ...
  primaryTag:{ fontSize: 8, fontWeight: 700, padding: "1px 6px", borderRadius: 3, background: "#444", marginLeft: 6 },
  ...
  changePassBtn:{ width: "100%", padding: "8px", background: "transparent", border: "1px solid #1e1e2e", borderRadius: 8, fontSize: 12, color: "#444", cursor: "pointer", fontFamily: "inherit", marginBottom: 6 },
